...
- 精确对齐当前 v3.8 功能导图中的13项 AG-002 能力；样例阶段实际演示安全要点、风险/禁忌/合规提示、操作步骤、术语转换、场景化指引、摘要和原文定位。
...
- 每个 Agent 使用独立 `Provider Registry` 注入 `AIPlatformPort`、`BusinessDataPort` 或 `DocumentDataPort`；AG-002 Provider 延迟解析并隔离配置故障，不影响其他 Agent 和页面启动。
